...
def temperature_prompt_activity():
    ...
    custom = input("Enter your instructions-based prompt:").strip()
    try:
        temp = float(input("Set temperature-based prompt: ").strip())
        if not (0.1 <= temp <= 1.0): raise ValueError
    except ValueError:
        print("Invalid temperature. Using 0.7.")
        temp = 0.7

    print(f"\n--- YOUR PROMT @ TEMP {temp} ---")
    ...
    print("Generate content -> rewrite with constraints -> create a sequel 9 try different temps).")
